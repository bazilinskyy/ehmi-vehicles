...
if __name__ == '__main__':
    ...
    files_heroku = ar.common.get_configs('files_heroku')
    heroku = ar.analysis.Heroku(files_data=files_heroku, save_p=SAVE_P, load_p=LOAD_P, save_csv=SAVE_CSV)
    # read heroku data
    ...
    files_appen = ar.common.get_configs('files_appen')
    appen = ar.analysis.Appen(files_data=files_appen, save_p=SAVE_P, load_p=LOAD_P, save_csv=SAVE_CSV)
    # read appen data
    ...
    if REJECT_CHEATERS:
        qa = ar.analysis.QA(file_cheaters=ar.common.get_configs('file_cheaters'),
                            job_id=ar.common.get_configs('appen_job'))
        qa.reject_users()
        qa.ban_users()
    # get data for lab and crowd samples separately
    if ar.common.get_configs('separate_lab'):
        # get data only from the lab and crowd samples
        heroku_data_lab = heroku_data[heroku_data['worker_code'].sar.contains(r'\blab_.*', regex=True)]
        appen_data_lab = appen_data[appen_data['worker_code'].sar.contains(r'\blab_.*', regex=True)]
        heroku_data_crowd = heroku_data[~heroku_data['worker_code'].sar.contains(r'\blab_.*', regex=True)]
        appen_data_crowd = appen_data[~appen_data['worker_code'].sar.contains(r'\blab_.*', regex=True)]
    ...
    if CALC_COORDS:
        ...
        ar.common.save_to_p('coords.p', [points, points_duration], 'points data')
        # calculate coordinates for lab and crowd samples separately
        if ar.common.get_configs('separate_lab'):
            points_lab, _, points_duration_lab = heroku.points(heroku_data_lab)
            ar.common.save_to_p('coords_lab.p', [points_lab, points_duration_lab], 'points data lab')
            points_crowd, _, points_duration_crowd = heroku.points(heroku_data_crowd)
            ar.common.save_to_p('coords_crowd.p', [points_crowd, points_duration_crowd], 'points data crowd')
    else:
        points, points_duration = ar.common.load_from_p('coords.p', 'points data')
        # load coordinates for lab and crowd samples separately
        if ar.common.get_configs('separate_lab'):
            points_lab, points_duration_lab = ar.common.load_from_p('coords_lab.p', 'points data lab')
            points_crowd, points_duration_crowd = ar.common.load_from_p('coords_crowd.p', 'points data crowd')
    ...
    if UPDATE_MAPPING:
        ...
        # export to pickle
        ar.common.save_to_p('mapping.p',  mapping, 'mapping of stimuli')
        ...
        mapping = ar.common.load_from_p('mapping.p', 'mapping of stimuli')
    # Output
    if SHOW_OUTPUT:
        analysis = ar.analysis.Analysis()
        ...
        if SHOW_OUTPUT_KP:
            ...
            #                  conf_interval=0.95,
            #                  save_file=True,
            #                  save_final=ar.common.get_configs('save_figures'))
            ...
            for stim in tqdm(range(int(num_stimuli/4))):  # tqdm adds progress bar
                ...
                analysis.plot_kp_slider_videos(df,
                                               ...
                                               events_width=1,
                                               events_dash='dot',
                                               events_colour='white' if ar.common.get_configs('plotly_template') == 'plotly_dark' else 'black',  # noqa: E501
                                               events_annotations_font_size=12,
                                               events_annotations_colour='white' if ar.common.get_configs('plotly_template') == 'plotly_dark' else 'black',  # noqa: E501
                                               ...
                                               ttest_marker='circle',
                                               ttest_marker_size=3,
                                               ttest_marker_colour='white' if ar.common.get_configs('plotly_template') == 'plotly_dark' else 'black',  # noqa: E501
                                               ttest_annotations_font_size=10,
                                               ttest_annotations_colour='white' if ar.common.get_configs('plotly_template') == 'plotly_dark' else 'black',  # noqa: E501
                                               anova_signals=anova_signals,
                                               anova_marker='cross',
                                               anova_marker_size=3,
                                               anova_marker_colour='white' if ar.common.get_configs('plotly_template') == 'plotly_dark' else 'black',  # noqa: E501
                                               anova_annotations_font_size=10,
                                               anova_annotations_colour='white' if ar.common.get_configs('plotly_template') == 'plotly_dark' else 'black',  # noqa: E501,
                ...
                signal3 = ar.common.vertical_sum(mapping.loc[mapping['id'].isin(ids)]['kp_raw'].iloc[0])
                # perform test
            ...
            # prepare pairs of signals to compare with ttest
            ttest_signals = [{'signal_1': ar.common.vertical_sum(mapping.loc[mapping['ego_car'] == 0]['kp_raw'].iloc[0]),  # noqa: E501
                              'signal_2': ar.common.vertical_sum(mapping.loc[mapping['ego_car'] == 1]['kp_raw'].iloc[0]),  # noqa: E501
                              'label': 'ttest(AV, MDV)',
                              'paired': True}]
            # prepare signals to compare with oneway ANOVA on the res level
            # anova_signals = [{'signals': [ar.common.vertical_sum(mapping.loc[mapping['ego_car'] == 0]['kp_raw'].iloc[0]),   # keypress data  # noqa: E501
            #                               ar.common.vertical_sum(mapping.loc[mapping['ego_car'] == 1]['kp_raw'].iloc[0])],  # noqa: E501
            #                   'label': 'anova'}]
            anova_signals = [{'signal_1': mapping.loc[mapping['id'].isin(ids)]['target_car'].tolist(),  # noqa: E501
                              'signal_2': mapping.loc[mapping['id'].isin(ids)]['ego_car'].tolist(),  # noqa: E501
                              'signal_3': [ar.common.vertical_sum(mapping.loc[mapping['ego_car'] == 0]['kp_raw'].iloc[0]),   # keypress data  # noqa: E501
                                           ar.common.vertical_sum(mapping.loc[mapping['ego_car'] == 1]['kp_raw'].iloc[0])],  # noqa: E501
                              'label': 'anova'}]               
            ...
            signal1 = mapping.loc[mapping['id'].isin(ids)]['target_car'].tolist()
            signal2 = mapping.loc[mapping['id'].isin(ids)]['ego_car'].tolist()
            signal3 = ar.common.vertical_sum(mapping.loc[mapping['ego_car'] == 0]['kp_raw'].iloc[0])
            # perform test
            analysis.twoway_anova_kp_whole(signal1, signal2, signal3, output_console=True)
            # keypress based on the type of ego car
            # prepare pairs of signals to compare with ttest
            # ttest_signals = [{'signal_1': ar.common.vertical_sum(mapping.loc[mapping['target_car'] == 0]['kp_raw'].iloc[0]),  # noqa: E501
            #                   'signal_2': ar.common.vertical_sum(mapping.loc[mapping['target_car'] == 1]['kp_raw'].iloc[0]),  # noqa: E501
            #                   'label': 'ttest(AV, MDV)',
            #                   'paired': True}]
            # prepare signals to compare with oneway ANOVA on the res level
            anova_signals = [{'signal_1': mapping.loc[mapping['id'].isin(ids)]['target_car'].tolist(),  # noqa: E501
                              'signal_2': mapping.loc[mapping['id'].isin(ids)]['ego_car'].tolist(),  # noqa: E501
                              'signal_3': [ar.common.vertical_sum(mapping.loc[mapping['target_car'] == 0]['kp_raw'].iloc[0]),   # keypress data  # noqa: E501
                                           ar.common.vertical_sum(mapping.loc[mapping['target_car'] == 1]['kp_raw'].iloc[0])],  # noqa: E501
                              'label': 'anova'}]
            # todo: double check that order of AV/MDV is correct
            # plot keypress data
            analysis.plot_kp_variable(mapping,
            ...
            signal1 = mapping.loc[mapping['id'].isin(ids)]['target_car'].tolist()
            signal2 = mapping.loc[mapping['id'].isin(ids)]['ego_car'].tolist()
            signal3 = ar.common.vertical_sum(mapping.loc[mapping['target_car'] == 0]['kp_raw'].iloc[0])
            # perform test
            analysis.twoway_anova_kp_whole(signal1, signal2, signal3, output_console=True)
            # keypress based on the pp group
            # prepare pairs of signals to compare with ttest
            ttest_signals = [{'signal_1': ar.common.vertical_sum(mapping.loc[mapping['group'] == 0]['kp_raw'].iloc[0]),  # noqa: E501
                              'signal_2': ar.common.vertical_sum(mapping.loc[mapping['group'] == 1]['kp_raw'].iloc[0]),  # noqa: E501
                              'label': 'ttest(0, 1)',
                              'paired': True},
                             {'signal_1': ar.common.vertical_sum(mapping.loc[mapping['group'] == 0]['kp_raw'].iloc[0]),  # noqa: E501
                              'signal_2': ar.common.vertical_sum(mapping.loc[mapping['group'] == 2]['kp_raw'].iloc[0]),  # noqa: E501
                              'label': 'ttest(0, 2)',
                              'paired': True},
                             {'signal_1': ar.common.vertical_sum(mapping.loc[mapping['group'] == 0]['kp_raw'].iloc[0]),  # noqa: E501
                              'signal_2': ar.common.vertical_sum(mapping.loc[mapping['group'] == 3]['kp_raw'].iloc[0]),  # noqa: E501
                              'label': 'ttest(0, 3)',
                              'paired': True},
                             {'signal_1': ar.common.vertical_sum(mapping.loc[mapping['group'] == 1]['kp_raw'].iloc[0]),  # noqa: E501
                              'signal_2': ar.common.vertical_sum(mapping.loc[mapping['group'] == 2]['kp_raw'].iloc[0]),  # noqa: E501
                              'label': 'ttest(1, 2)',
                              'paired': True},
                             {'signal_1': ar.common.vertical_sum(mapping.loc[mapping['group'] == 2]['kp_raw'].iloc[0]),  # noqa: E501
                              'signal_2': ar.common.vertical_sum(mapping.loc[mapping['group'] == 3]['kp_raw'].iloc[0]),  # noqa: E501
                              'label': 'ttest(2, 3)',
                              'paired': True},
                             {'signal_1': ar.common.vertical_sum(mapping.loc[mapping['group'] == 1]['kp_raw'].iloc[0]),  # noqa: E501
                              ...
                              'paired': True}]
            # prepare signals to compare with oneway ANOVA on the res level
            # anova_signals = [{'signals': [ar.common.vertical_sum(mapping.loc[mapping['group'] == 0]['kp_raw'].iloc[0]),   # keypress data  # noqa: E501
            #                               ar.common.vertical_sum(mapping.loc[mapping['group'] == 1]['kp_raw'].iloc[0]),   # noqa: E501
            #                               ar.common.vertical_sum(mapping.loc[mapping['group'] == 2]['kp_raw'].iloc[0]),   # noqa: E501
            #                               ar.common.vertical_sum(mapping.loc[mapping['group'] == 3]['kp_raw'].iloc[0])],  # noqa: E501
            #                   'label': 'anova'}]
            # prepare signals to compare with oneway ANOVA on the res level
            anova_signals = [{'signal_1': mapping.loc[mapping['id'].isin(ids)]['target_car'].tolist(),  # noqa: E501
                              'signal_2': mapping.loc[mapping['id'].isin(ids)]['ego_car'].tolist(),  # noqa: E501
                              'signal_3': [[ar.common.vertical_sum(mapping.loc[mapping['group'] == 0]['kp_raw'].iloc[0]),   # keypress data  # noqa: E501
                                            ar.common.vertical_sum(mapping.loc[mapping['group'] == 1]['kp_raw'].iloc[0]),   # noqa: E501
                                            ar.common.vertical_sum(mapping.loc[mapping['group'] == 2]['kp_raw'].iloc[0]),   # noqa: E501
                                            ar.common.vertical_sum(mapping.loc[mapping['group'] == 3]['kp_raw'].iloc[0])]],  # noqa: E501
                              'label': 'anova'}]
            ...
            signal1 = mapping.loc[mapping['id'].isin(ids)]['target_car'].tolist()
            signal2 = mapping.loc[mapping['id'].isin(ids)]['ego_car'].tolist()
            signal3 = ar.common.vertical_sum(mapping.loc[mapping['group'] == 0]['kp_raw'].iloc[0])
            # perform test
            analysis.twoway_anova_kp_whole(signal1, signal2, signal3, output_console=True)
        ...
        if SHOW_OUTPUT_ST:
            ...
            analysis.hist(heroku_data,
                          x=heroku_data.columns[heroku_data.columns.to_series().sar.contains('-dur')],
                          nbins=100,
                          pretty_text=True,
                          save_file=True,
                          save_final=ar.common.get_configs('save_figures'))
        ...
        if SHOW_OUTPUT_ET:
            ...
            logger.info('Producing visualisations of eye gaze data for {} stimuli.',
                        ar.common.get_configs('num_stimuli'))
            if ar.common.get_configs('combined_animation') == 1:
                ...
                num_anim = ar.common.get_configs('num_stimuli')
            ...
            for id_video in tqdm(range(0, num_anim)):
                ...
                stim_path = os.path.join(ar.settings.output_dir, 'frames')
                ...
                hm_resolution_range = int(50000 / ar.common.get_configs('hm_resolution'))
