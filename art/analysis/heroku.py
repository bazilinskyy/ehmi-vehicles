...
class Heroku:
    ...
    mapping = pd.read_csv(ar.common.get_configs('mapping_stimuli'))
    # number of stimuli
    num_stimuli = ar.common.get_configs('num_stimuli')
    # number of stimuli shown for each participant
    num_stimuli_participant = ar.common.get_configs('num_stimuli_participant')
    # number of repeats for each stimulus
    num_repeat = ar.common.get_configs('num_repeat')
    ...
    def __init__(self,
        ...
        self.num_stimuli = ar.common.get_configs('num_stimuli')
    ...
    def read_data(self, filter_data=True):
        ...
        # load data
        if self.load_p:
            df = ar.common.load_from_p(self.file_p, 'heroku data')
        # process data
        else:
            ...
            for row in tqdm(data_list):  # tqdm adds progress bar
                ...
                for data_cell in list_row['data']:
                    ...
                    if 'stimulus' in data_cell.keys():
                        ...
                        # instructions block
                        if (ar.common.search_dict(self.prefixes, stim_no_path)
                                is not None):
                            # stimulus is found
        ...
        # save to pickle
        if self.save_p:
            ar.common.save_to_p(self.file_p, df, 'heroku data')
        # save to csv
        if self.save_csv:
            # build path
            if not os.path.exists(ar.settings.output_dir):
                os.makedirs(ar.settings.output_dir)
            # save to file
            df.to_csv(os.path.join(ar.settings.output_dir, self.file_data_csv), index=False)
    ...
    def read_mapping(self):
        ...
        df = pd.read_csv(ar.common.get_configs('mapping_stimuli'))
    ...
    def points(self, df, save_csv=True):
        ...
        hm_resolution = ar.common.get_configs('hm_resolution')
        # dictionaries to store points
        points = {}
        points_worker = {}
        points_duration = [{} for x in range(0, 5000000000, hm_resolution)]
        # window values for normalization
        height = int(ar.common.get_configs('stimulus_height'))
        width = int(ar.common.get_configs('stimulus_width'))
        ...
        if save_csv:
            ...
            # # save to csv
            # df_csv.to_csv(ar.settings.output_dir + '/' +
    ...
    def process_kp(self, df, filter_length=True, kp_col='kp', kp_raw_col='kp_raw'):
        ...
        # save to csv
        if self.save_csv:
            self.mapping.to_csv(os.path.join(ar.settings.output_dir, self.file_mapping_csv))
        # return new mapping
        return self.mapping
    ...
    def process_stimulus_questions(self, questions):
        ...
        Args:
            questions (list): list of questions with types of possible values as int or sar.
