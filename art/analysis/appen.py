...
class Appen:
    ...
    def read_data(self, filter_data=True, clean_data=True):
        ...
        # load data
        if self.load_p:
            df = ar.common.load_from_p(self.file_p, 'appen data')
        # process data
        else:
            ...
            df.columns = df.columns.sar.lstrip('_')
            # clean data
            if clean_data:
        ...
        # save to pickle
        if self.save_p:
            ar.common.save_to_p(self.file_p, df, 'appen data')
        # save to csv
        if self.save_csv:
            ...
            # save to file
            df.to_csv(os.path.join(ar.settings.output_dir, self.file_csv))
    ...
    def filter_data(self, df):
        ...
        # People that took less than ar.common.get_configs('allowed_min_time')
        # minutes to complete the study
        df_4 = df.loc[df['time'] < ar.common.get_configs('allowed_min_time')]
        logger.info('Filter-a4. People who completed the study in under ' +
                    str(ar.common.get_configs('allowed_min_time')) +
                    ' sec: {}',
                    df_4.shape[0])
        ...
        if self.save_csv:
            ...
            df_6.to_csv(os.path.join(ar.settings.output_dir, self.file_cheaters_csv))
    ...
    def mask_ips_ids(self, df, mask_ip=True, mask_id=True):
        ...
        given ID from ar.common.get_configs('mask_id').
        """
        ...
        for i in range(len(df['ip'])):  # loop through ips
            ...
            if mask_id:
                ...
                if not any(d['o'] == df['worker_id'][i] for d in proc_ids):
                    ...
                    masked_id = (str(ar.common.get_configs('mask_id') - df['worker_id'][i]))
                    # record IP as already replaced
    ...
    def process_countries(self, df):
        ...
        # save to csv
        if self.save_csv:
            df_country.to_csv(os.path.join(ar.settings.output_dir, self.file_country_csv))
