...
class Analysis:
    ...
    template = ar.common.get_configs('plotly_template')
    # number of stimuli
    num_stimuli = ar.common.get_configs('num_stimuli')
    fig = None
    g = None
    image = None
    stim_id = None
    ...
    def __init__(self):
        # set font globally
        plt.rcParams['font.family'] = ar.common.get_configs('font_family')
    ...
    Returns:
        ...
        path = os.path.join(ar.settings.output_dir, 'frames')
        # create temp folder
        if not os.path.exists(path):
            os.makedirs(path)
        # video file in the folder with stimuli
        cap = cv2.VideoCapture(
            os.path.join(ar.common.get_configs('path_stimuli'), 'video_' + str(id_video) + '.mp4'))
        # timestamp
        t = mapping.loc['video_' + str(id_video)][t]
        self.time = int(t)
        self.hm_resolution = ar.common.get_configs('hm_resolution')
        hm_resolution_int = int(ar.common.get_configs('hm_resolution'))
    ...
    Returns:
        ...
        width = ar.common.get_configs('stimulus_width')
        height = ar.common.get_configs('stimulus_height')
    ...
    Args:
        ...
        """
        self.image = image
        self.hm_resolution_range = int(50000/ar.common.get_configs('hm_resolution'))
        ...
        aoi = pd.read_csv(ar.common.get_configs('aoi'))
    ...
    Args:
        ...
        # create path
        path = os.path.join(ar.settings.output_dir, self.folder_figures)
        if not os.path.exists(path):
            os.makedirs(path)
    ...
    Returns:
        ...
        if ar.common.get_configs('combined_animation') == 1:
            self.g[0].plot(np.array(self.times[:it]),
        ...
        if ar.common.get_configs('only_lab') == 1:
        ...
        if self.save_frames:
            ...
            # save figure
            self.save_fig(self.image, temp_fig, os.path.join(ar.settings.output_dir, self.folder_figures), name)
        return self.g
        # save each frame as file
        if self.save_frames:
            # build filename
    ...
    def scatter_matrix(self, df, columns_drop, color=None, symbol=None, diagonal_visible=False, xaxis_title=None,
        ...
        if font_family:
            ...
            fig.update_layout(font=dict(family=ar.common.get_configs('font_family')))
        # update font size
        if font_size:
            # use given value
            ...
            # use value from config file
            fig.update_layout(font=dict(size=ar.common.get_configs('font_size')))
        # save file to local output folder
        if save_file:
        ...
        if pretty_text:
            for variable in y:
                ...
                if isinstance(df.iloc[0][variable], str):
                    ...
                    df[variable] = df[variable].sar.replace('_', ' ')
                    # capitalise
                    df[variable] = df[variable].sar.capitalize()
    ...
    def scatter(self, df, x, y, color=None, symbol=None, size=None, text=None, trendline=None, hover_data=None,
        ...
        if pretty_text:
            if isinstance(df.iloc[0][x], str):  # check if string
                # replace underscores with spaces
                df[x] = df[x].sar.replace('_', ' ')
                # capitalise
                df[x] = df[x].sar.capitalize()
            if isinstance(df.iloc[0][y], str):  # check if string
                # replace underscores with spaces
                df[y] = df[y].sar.replace('_', ' ')
                # capitalise
                df[y] = df[y].sar.capitalize()
            if color and isinstance(df.iloc[0][color], str):  # check if string
                # replace underscores with spaces
                df[color] = df[color].sar.replace('_', ' ')
                # capitalise
                df[color] = df[color].sar.capitalize()
            if size and isinstance(df.iloc[0][size], str):  # check if string
                # replace underscores with spaces
                df[size] = df[size].sar.replace('_', ' ')
                # capitalise
                df[size] = df[size].sar.capitalize()
            try:
                # check if string
                if text and isinstance(df.iloc[0][text], str):
                    # replace underscores with spaces
                    df[text] = df[text].sar.replace('_', ' ')
                    # capitalise
                    df[text] = df[text].sar.capitalize()
        ...
        if pretty_text:
            for x_col in x:
                if isinstance(df.iloc[0][x_col], str):  # check if string
                    # replace underscores with spaces
                    df[x_col] = df[x_col].sar.replace('_', ' ')
                    # capitalise
                    df[x_col] = df[x_col].sar.capitalize()
                else:
    ...
    def heatmap(self, df, x, y, t, id_video, pp, pretty_text=False, marginal_x='violin', marginal_y='violin',
        ...
        if pretty_text:
            ...
            if isinstance(df.iloc[0][t], str):  # check if string
                # replace underscores with spaces
                df[t] = df[t].sar.replace('_', ' ')
                # capitalise
                df[t] = df[t].sar.capitalize()
        pp = str(pp)
    ...
    def hist_stim_duration_time(self, df, time_ranges, nbins=0, font_family=None, font_size=None, name_file=None,
        ...
        col_dur = df.columns[df.columns.to_series().sar.contains('-dur')]
    ...
    def save_plotly(self, fig, name, remove_margins=False, width=1320, height=680, save_eps=True, save_png=True,
        ...
        path_final = os.path.join(ar.settings.root_dir, self.folder_figures)
    ...
    def save_fig(self, fig, name, remove_margins=False, pad_inches=0, save_final=False):
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        # build path for final figure
        path_final = os.path.join(ar.settings.output_dir, self.folder_figures)
        if save_final and not os.path.exists(path_final):
            os.makedirs(path_final)
        # limit name to max 200 char (for Windows)
        if len(path) + len(name) > 195 or len(path_final) + len(name) > 195:
    ...
    def smoothen_filter(self, signal, type_flter='OneEuroFilter'):
        ...
        if type_flter == 'OneEuroFilter':
            filter_kp = OneEuroFilter(freq=ar.common.get_configs('freq'),            # frequency
                                      mincutoff=ar.common.get_configs('mincutoff'),  # minimum cutoff frequency
                                      beta=ar.common.get_configs('beta'))            # beta value
    ...
    def ttest(self, signal_1, signal_2, type='two-sided', paired=True):
        ...
        significance = []  # record binary flag (0 or 1) if p value < ar.common.get_configs('p_value'))
        ...
        for i in range(len(signal_1)):
            ...
            significance.append(int(p_value < ar.common.get_configs('p_value')))
    ...
    def anova(self, signals):
        ...
        significance = []  # record binary flags (0 or 1) if p-value < ar.common.get_configs('p_value')
    ...
    def save_stats_csv(self, t, p_values, name_file):
        ...
        """
        path = os.path.join(ar.settings.output_dir, self.folder_stats)  # where to save csv
        # build path
        if not os.path.exists(path):
            os.makedirs(path)
    ...
    @staticmethod
    def load_test_data(path, stimuli_base_path=None):
        """Parse test_data.json (NDJSON) and return gaze data grouped by stimulus.

        Gaze coordinates in webgazer_data are in screen pixels. This method
        normalises them to the stimulus image coordinate system using the
        webgazer_targets bounding box recorded for each trial.

        Args:
            path (str): path to the NDJSON file (one JSON object per line).
            stimuli_base_path (str, optional): root directory from which the
                relative stimulus paths in the data resolve to actual image
                files (e.g. '/path/to/project/public').  If given, an
                'image_path' key is added to each entry.

        Returns:
            dict: mapping stimulus name (str) to a dict with keys:
                - 'gaze_points' (list of [x, y]): gaze coordinates in image
                  space (pixels relative to the top-left of the image).
                - 'gaze_raw' (list of {x, y, t}): original screen-space gaze
                  with timestamp (ms since trial start).
                - 'participant_count' (int): number of participant trials.
                - 'image_path' (str or None): resolved path to the image file.
        """
        import json as _json

        KEY = '#jspsych-image-keyboard-response-stimulus'
        ...
        data = defaultdict(lambda: {'gaze_points': [], 'gaze_raw': [], 'participant_count': 0, 'image_path': None})
        ...
        for record in records:
            for trial in record.get('data', []):
                ...
                bbox = targets.get(KEY)

                # extract stimulus name from path list
                ...
                # normalise to image-relative coordinates if bounding box available
                if bbox:
                ...
                # resolve image path
                if stimuli_base_path and entry['image_path'] is None:
                    candidate = os.path.join(stimuli_base_path, stim)
                    entry['image_path'] = candidate if os.path.exists(candidate) else None

        return dict(data)

    def plot_et_heatmap_image(self, stimulus, gaze_points, image_path=None, type_heatmap='kdeplot',
                              alpha=0.5, show_scatter=False, scatter_alpha=0.3, scatter_size=4,
                              title=None, name_file=None, save_file=False, save_final=False,
                              fig_save_width=1320, fig_save_height=680):
        """Overlay an eye-tracking heatmap on a stimulus image.

        Gaze coordinates should be in image-relative pixels (as returned by
        load_test_data).  If no image_path is provided, the heatmap is drawn
        on a plain white background sized to the data extent.

        Args:
            stimulus (str): stimulus name used in log messages and filenames.
            gaze_points (list): list of [x, y] gaze coordinates in image space.
            image_path (str, optional): path to the stimulus image file.
            type_heatmap (str, optional): 'kdeplot', 'contourf', or 'hist2d'.
            alpha (float, optional): opacity of the heatmap overlay.
            show_scatter (bool, optional): overlay individual gaze points.
            scatter_alpha (float, optional): opacity for scatter points.
            scatter_size (int, optional): marker size for scatter points.
            title (str, optional): figure title.  Defaults to stimulus name.
            name_file (str, optional): output filename stem.
            save_file (bool, optional): save figure to disk.
            save_final (bool, optional): also save to root /figures folder.
            fig_save_width (int, optional): saved figure width in pixels.
            fig_save_height (int, optional): saved figure height in pixels.
        """
        logger.info('Creating eye-tracking heatmap for stimulus {}.', stimulus)

        if not gaze_points:
            logger.error('No gaze points for stimulus {}.', stimulus)
            return

        xy = np.array(gaze_points)
        x, y = xy[:, 0], xy[:, 1]
        ...
        if img is not None:
            ax.imshow(img, extent=[0, img_w, img_h, 0])  # origin top-left

        if type_heatmap == 'kdeplot':
            try:
                sns.kdeplot(x=x, y=y, ax=ax, alpha=alpha, fill=True, cmap='RdBu_r', bw_adjust=0.5)
            except Exception as e:
                logger.error('KDE plot failed for {}: {}', stimulus, e)
        elif type_heatmap == 'contourf':
            try:
                k = gaussian_kde(np.vstack([x, y]))
                xi, yi = np.mgrid[x.min():x.max():x.size**0.5*1j, y.min():y.max():y.size**0.5*1j]
                zi = k(np.vstack([xi.flatten(), yi.flatten()]))
                ...
                logger.error('Contourf plot failed for {}: {}', stimulus, e)
        elif type_heatmap == 'hist2d':
            bins = [max(10, img_w // 50), max(10, img_h // 50)]
            ax.hist2d(x, y, bins=bins, cmap='jet', alpha=alpha)
        else:
            logger.error('Unknown type_heatmap {} for plot_et_heatmap_image.', type_heatmap)
            plt.close(fig)
            return

        if show_scatter:
            ax.scatter(x, y, alpha=scatter_alpha, s=scatter_size, c='white', edgecolors='none')
        ...
        if save_file:
            if not name_file:
                name_file = 'et_heatmap_' + stimulus
            self.save_fig(fig=fig,
                          name=name_file,
